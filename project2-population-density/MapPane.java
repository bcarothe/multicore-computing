...
public class MapPane extends JPanel implements Observer {
	...
	// for debugging: we placed these cities on the canvas and then lined the
	// background image to match.

	/*
	 * private final double MIAMI_N1 = 25.787778; private final double MIAMI_N =
	 * mercatorConversion(MIAMI_N1); private final double MIAMI_E = -80.224167;
	 * private final double SEATTLE_N1 = 47.61; private final double SEATTLE_N =
	 * mercatorConversion(SEATTLE_N1); private final double SEATTLE_E = -122.33;
	 * private final double BOSTON_N1 = 42.34711; private final double BOSTON_N =
	 * mercatorConversion(BOSTON_N1); private final double BOSTON_E = -71.0945;
	 * private final double SANDIEGO_N1 = 32.71528; private final double SANDIEGO_N
	 * = mercatorConversion(SANDIEGO_N1); private final double SANDIEGO_E =
	 * -117.15722; private final double HONOLULU_N1 = 21.3069; private final double
	 * HONOLULU_N = mercatorConversion(HONOLULU_N1); private final double HONOLULU_E
	 * = -157.8583;
	 */
	...
	public void paint(Graphics g) {
		...
		// For debugging: Draw some cities on top of the background image. Then line the
		// image up with these cities.
		/*
		 * double nsRatio = 1.0 * height / (MAX_NORTH - MAX_SOUTH); double ewRatio = 1.0
		 * * width / (MAX_EAST - MAX_WEST); Rectangle2D seattle = new
		 * Rectangle2D.Double(-1 * (MAX_WEST - SEATTLE_E) * ewRatio, (MAX_NORTH -
		 * SEATTLE_N) * nsRatio, 6, 6); Rectangle2D boston = new Rectangle2D.Double(-1 *
		 * (MAX_WEST - BOSTON_E) * ewRatio, (MAX_NORTH - BOSTON_N) * nsRatio, 6, 6);
		 * Rectangle2D sandiego = new Rectangle2D.Double(-1 * (MAX_WEST - SANDIEGO_E) *
		 * ewRatio, (MAX_NORTH - SANDIEGO_N) * nsRatio, 6, 6); Rectangle2D miami = new
		 * Rectangle2D.Double(-1 * (MAX_WEST - MIAMI_E) * ewRatio, (MAX_NORTH - MIAMI_N)
		 * * nsRatio, 6, 6); Rectangle2D honolulu = new Rectangle2D.Double(-1 *
		 * (MAX_WEST - HONOLULU_E) * ewRatio, (MAX_NORTH - HONOLULU_N) * nsRatio, 6, 6);
		 * g2.setColor(Color.RED); g2.fill(seattle); g2.fill(sandiego); g2.fill(boston);
		 * g2.fill(miami); g2.fill(honolulu); g2.setColor(Color.BLACK);
		 */
